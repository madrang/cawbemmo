module.exports = {
	...
	, init: function (blueprint) {
		this.passiveTree = require("../config/passiveTree");

		this.selected = ((blueprint || {}).selected || []);
		this.selected.spliceWhere((s) => (this.passiveTree.nodes.some((n) => ((n.id === s) && (n.spiritStart)))));
		this.selected.push(this.passiveTree.nodes.find((n) => (n.spiritStart === this.obj.class)).id);
		...
		this.selected.forEach((id) => {
			let node = this.passiveTree.nodes.find((n) => (n.id === id));
			if (node) {
	...
	, getTree: function (msg) {
		//_.log.passives.debug("Sending passives skill tree.");
	...
	, tickNode: function (msg) {
		...
		let node = this.passiveTree.nodes.find((n) => (n.id === nodeId));

		if (!node || node.spiritStart) {
			return;
		}

		let linked = this.passiveTree.links.some((l) => {
			if (l.from !== node.id && l.to !== node.id) {
				return false;
			}
			return (this.selected.includes(l.from) || this.selected.includes(l.to));
		}, this);
		if (!linked) {
			return;
		}
	...
	, untickNode: function (msg) {
		...
		this.selected.forEach((s) => {
			let node = this.passiveTree.nodes.find((n) => (n.id === s));
			if (node.spiritStart) {
				return;
			}
